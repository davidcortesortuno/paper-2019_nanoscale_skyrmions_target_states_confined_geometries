...
mkdir -p ndts
for (( B = 600; B < 1201; B += 100 )); do
    SIMNAME="nebm_2Dhex_image-rotated_PdFe-Ir_tgt-st-up_sk-down_B${B}mT"
    #
    python ../hexagonal_neb_fidimag.py \
        ...
        --mu_s $MU_S \
        --k_u 0.406 \
        --interpolation "../../relaxation/image_rotated_B-sweep_DT/npys/2Dhex_image-rotated_PdFe-Ir_tgt-st-up_B${B}mT_npys/" \
                        28 \
                        "../../relaxation/image_rotated_B-sweep_DT/npys/2Dhex_image-rotated_PdFe-Ir_1-sk-down_B${B}mT_npys/" \
        --nebm_k 1e4 \
